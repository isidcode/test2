      ...
      <section className="h-cover md:flex flex-row-reverse items-start gap-5 min-[1100px]:gap-12">
        ...
        <div className="flex flex-col max-md:items-center gap-5 min-w-[250px] md:w-[50%] md:pl-8 md:border-l border-grey md:sticky md:top-[100px] md:py-10">
          ...
          <img
            ...
            alt={personal_info.username}
          />

          <h1 className="text-2xl font-medium">@{personal_info.username}</h1>
          <p className="text-xl capitalize h-6">{personal_info.fullname}</p>
          <p className="text-dark-grey">{personal_info.bio}</p>

          {/* Stats */}
          <div className="flex gap-8 my-3">
            <div className="text-center">
              <h1 className="text-xl font-medium">{account_info?.total_posts ?? 0}</h1>
              <p className="text-dark-grey capitalize">Blogs Published</p>
            </div>
            <div className="text-center">
              <h1 className="text-xl font-medium">{account_info?.total_reads ?? 0}</h1>
          ...
          {/* Social links */}
          {Object.entries(social_links).some(([, v]) => v) && (
        ...
        <div className="w-full">
          ...
          {blogs === null ? (
            ...
            <p className="text-dark-grey text-center mt-10">No blogs published yet.</p>
          ) : (
            blogs.map((blog, i) => (
              <AnimationWrapper key={i} transition={{ duration: 1, delay: i * 0.08 }}>
                <Link
                  to={`/blog/${blog.blog_id}`}
                  className="flex gap-6 items-center border-b border-grey pb-5 mb-4 hover:bg-grey/20 rounded-lg p-2 transition-all"
                >
                  <div className="w-full">
